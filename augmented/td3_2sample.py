    ...
    for time_step in range(max_time_step):
        with torch.no_grad():
            if len(Action_buffer) < 3: # Padding with 0
                ...
                I = torch.concatenate([state.unsqueeze(0), torch.FloatTensor([Action_buffer[-2], Action_buffer[-1]]).unsqueeze(0).to(device)] , dim = 1)
        ...
        if len(Action_buffer) < 3: # gathering actions
            ...
            next_state, reward, terminated, _, _ = current_env.step(Action_buffer[-2].cpu().detach().numpy())
            next_state = torch.tensor(next_state).float().to(device)
            I_next = torch.concatenate([next_state.unsqueeze(0), torch.FloatTensor([Action_buffer[-1], action]).unsqueeze(0).to(device)],dim=1)
            Action_buffer.append(action)
