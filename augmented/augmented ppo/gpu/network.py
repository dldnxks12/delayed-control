        ...
        for j in range(self.buffer_size):
            for i in range(self.minibatch_size):
                ...
                for transition in rollout:
                    ...
                    s_list.append(np.array(state))
                    a_list.append(np.array(action))
                    r_list.append([reward])
                    s_prime_list.append(np.array(next_state))
                    prob_list.append(np.array(prob))
                    done_mask = 0 if done else 1
                    done_list.append([done_mask])

                b_state.append(np.array(s_list))
                b_action.append(np.array(a_list))
                b_reward.append(r_list)
                b_next_state.append(np.array(s_prime_list))
                b_log_old_probs.append(np.array(prob_list))
                b_done.append(done_list)

            states  = torch.tensor(b_state, dtype=torch.float).to(self.device)
            actions = torch.tensor(b_action).to(self.device)
            rewards = torch.tensor(b_reward).to(self.device)
            next_states = torch.tensor(b_next_state, dtype=torch.float).to(self.device)
            probs = torch.tensor(b_log_old_probs).to(self.device)
            dones = torch.tensor(b_done, dtype=torch.float).to(self.device)
        ...
        if len(self.data) == self.minibatch_size * self.buffer_size:
            ...
            for _ in range(self.epochs):
                for mini_batch in data:
                    ...
                    surrogate1 = ratio * advantage.to(self.device)
                    surrogate2 = torch.clamp(ratio, 1-self.eps, 1+self.eps) * advantage.to(self.device)
