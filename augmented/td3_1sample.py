...
class PolicyNetwork(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(8, 128) # Input : State 3
...
for episode in range(MAX_EPISODE):
    ...
    Action_buffer = [torch.FloatTensor([0]).to(device)]

    # Generate Episodes ...
    for time_step in range(max_time_step):
        with torch.no_grad():
            I = torch.concatenate([state.unsqueeze(0), torch.FloatTensor([Action_buffer[-1]]).unsqueeze(0).to(device)] , dim = 1)

        action = Pi(I)[0]

        next_state, reward, terminated, _, _ = current_env.step(Action_buffer[-1].cpu().detach().numpy())
        next_state = torch.tensor(next_state).float().to(device)
        I_next = torch.concatenate([next_state.unsqueeze(0), torch.FloatTensor([action]).unsqueeze(0).to(device)],dim=1)
