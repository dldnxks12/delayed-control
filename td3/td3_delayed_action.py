...
class ReplayBuffer():
    ...
    def sample(self, batch_size):
        ...
        bstates, states, actions, rewards, next_states, terminateds, truncateds = [], [], [], [], [], [], []

        for bstate, state, action, reward, next_state, terminated, truncated in samples:
        ...
        return bstates, states, actions, rewards, next_states, terminateds, truncateds
...
def train(Buffer, Q1, Q1_target, Q2, Q2_target, Pi, Pi_target, Q1_optimizer, Q2_optimizer, Pi_optimizer):
    bstates, states, actions, rewards, next_states, terminateds, truncateds = Buffer.sample(batch_size)
    ...
    """
    with torch.no_grad():
        action_bar = Pi_target(states) # next_state : 32x3 , action_bar : 32 x 1
    ...
    pi_loss = - Q1(states, Pi(bstates)).mean() # bstates : 0
...
for episode in range(MAX_EPISODE):
    ...
    for time_step in range(max_time_step):
        ...
        if init_flag == True: # s0 -> a0 -> s1
            ...
            next_state, reward, terminated, truncated, _ = current_env.step(Action_buffer[-2].cpu().detach().numpy())
            next_state = torch.tensor(next_state).float().to(device)
        ...
        Buffer.put([State_buffer[-2], state, Action_buffer[-2], reward, next_state, terminated, truncated])
        total_reward += reward
...
Y = np.array(Y)
np.save('./TD3_3_action_delayed_history', Y)
